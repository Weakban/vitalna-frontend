    ...
    <>
      <Box paddingY={"2"} maxW="5xl">
        ...
        >
          ...
          <Box>
            <Card.Body p={4}>
              <Card.Title mb="2" fontWeight="semibold">
              ...
              <HStack mt="4">
                ...
                {typeof service.durationMin === "number" && (
                  <Badge variant="outline">{service.durationMin} min</Badge>
                )}

                {service.category && (
                  <Badge variant="subtle">{service.category.name}</Badge>
                )}

                <Badge colorScheme="purple">Rating ★★★★★</Badge>
              </HStack>
            </Card.Body>
                ...
                {mode === "mine" ? (
                  <>
                    <Button
                      ...
                      onClick={() =>
                        navigate(
                          `/app/professional/services/${service.id}/edit`
                        )
                      }
                    >
                      Editar
                  ...
                  <>
                    ...
                    <Checkbox.Root
                      ...
                      colorPalette="red"
                    >
                      <Checkbox.Label>Agrega a favoritos</Checkbox.Label>
...
<Box display="flex" flexDirection="column" justifyContent="space-between" p={4} flex="1">
  ...
  <HStack mt="4" gap={3}>
    ...
    {service.category && <Badge variant="subtle">{service.category.name}</Badge>}
    <Badge colorScheme="purple">★★★★★</Badge>
  </HStack>

  
  <HStack mt="6" gap={4} justify="flex-end">
    {mode === "mine" ? (
      <>
        <Button colorScheme="blue" onClick={() => navigate(`/app/professional/services/${service.id}/edit`)}>
          Editar <CiEdit />
        </Button>
        <Form method="post" action={`/app/professional/services/${service.id}/delete`}>
          <Button colorScheme="red" type="submit">Eliminar</Button>
        </Form>
      </>
    ) : (
      <>
        <Checkbox.Root size="lg" variant="subtle" defaultChecked colorPalette="red">
          <Checkbox.Label>Favorito</Checkbox.Label>
          <Checkbox.Control><FiHeart /></Checkbox.Control>
        </Checkbox.Root>
